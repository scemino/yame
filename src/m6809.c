...
extern int8_t Mgetc(uint16_t);       //pointer to function
extern void Mputc(uint16_t, int8_t); //pointer to function

int16_t Mgetw(uint16_t address) {
  return (Mgetc(address) << 8 | ((Mgetc(address + 1) & 0xff)));
}

void Mputw(uint16_t address, int16_t value) {
    Mputc(address, value >> 8);
    Mputc(address + 1, value);
}
